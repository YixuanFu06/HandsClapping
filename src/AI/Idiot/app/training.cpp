...
int main() {
    ...
    std::cout << "Train on model " << policy.GetName() << "[" << policy.GetId() << "] for " << total_rounds << " rounds:" << std::endl;
    for (uint32_t i = 0; i < total_rounds; i++) {
        ...
        while (battle_field.GetMemberNum() > 1) {
            ...
            battle_field.BattleFieldUpdate({action1->GetFormalName(), action2->GetFormalName()}, 1);
        }
    ...
    char confirm;
    std::cout << std::endl << "Do you want to store the policy? (y/n)" << std::endl;
    std::cin >> confirm;
    ...
    if (confirm == 'y') {
        ...
        std::cout << "Policy change is stored to " << data_path.string() << std::endl;
    } else if (confirm == 'n') {
